...
class BuildEquivalenceSpec extends FunSuite:
  ...
  test("can calculate classes from a set of representatives"):
    BuildEquivalence.toClasses(Seq()) is Seq()
    BuildEquivalence.toClasses(Seq(0)) is Seq(0)
    BuildEquivalence.toClasses(Seq(0, 1)) is Seq(0, 1)
    BuildEquivalence.toClasses(Seq(1, 0)) is Seq(0, 1)
    BuildEquivalence.toClasses(Seq(7, 6)) is Seq(0, 1)
    BuildEquivalence.toClasses(Seq(8, 2, 2, 3)) is Seq(0, 1, 1, 2)
    BuildEquivalence.toClasses(Seq(8, 0, 8, 6)) is Seq(0, 1, 0, 2)
    BuildEquivalence.toClasses(Seq(1, 5, 1, 7, 5, 5, 7)) is Seq(0, 1, 0, 2, 1, 1, 2)

  test("can calculate class indexes from a set of relators"):
